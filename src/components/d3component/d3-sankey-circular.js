...
import * as findCircuits from "elementary-circuits-directed-graph"
        ...
        columns.forEach(function (nodes) 
        {
          ...
          if (dy > 0) 
          {
            ...
            for (i = n - 2; i >= 0; --i) 
            {
              node = nodes[i];
              dy = node.y1 + py - y;
              if (dy > 0) return node.y0 -= dy, node.y1 -= dy;
              y = node.y0;
            }
          }
        });
 ...
 export  function addCircularPathData(graph, circularLinkGap, y1, id) {
  ...
  export const sankeyCenter = center;
  export const sankeyLeft = left;
  export const sankeyRight = right;
  export const sankeyJustify = justify;
